% =========================================================================
% Second example with synthetic data for endmember variability (piecewise linear functions)
% 
...
if SNR == 20
    load('DATA/data_ex2_20db.mat')
elseif SNR == 30
    load('DATA/data_ex2_30db.mat')
elseif SNR == 40
    load('DATA/data_ex2_40db.mat')
end
...
% Convert data back to cube
data = permute(reshape(Y',nc,nr,L),[2 1 3]); 

% Reorder data with column-wise lexicographic ordering
data_r = reshape(data,nr*nc,L);



%% Endmembers are loaded a priori for reproducibility, uncomment the following code to extract them again
%{
% Endmember initialization (VCA [1])
M0 = vca(Y,'Endmembers',P,'verbose','off');
...
axes(ha(16));
ylabel('MUASV','interpreter','latex')
colormap jet

%print('examples/estim_abundances_DC2_tght','-depsc')
